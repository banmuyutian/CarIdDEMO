...
public class PlateLocate {
    ...
    public Vector<Mat> plateLocate(Mat src) {
        ...
        functionControler.recordProcess("图像进行灰度化","tmp/debug_gray.jpg");

        // 高斯模糊。Size中的数字影响车牌定位的效果。
        GaussianBlur(src_gray, src_blur, new Size(gaussianBlurSize, gaussianBlurSize), 0, 0, BORDER_DEFAULT);

        opencv_imgcodecs.imwrite("tmp/debug_GaussianBlur.jpg", src_blur);
        functionControler.recordProcess("高斯模糊，图片降噪处理","tmp/debug_GaussianBlur.jpg");
        src_gray = src_blur;
        ...
        functionControler.recordProcess("使用Sobel算子进行边缘检测，得到的是图像的一阶水平方向导数","tmp/debug_Sobel.jpg");
        ...
        functionControler.recordProcess("对图像进行二值化。将灰度图像（每个像素点有256 个取值可能）转化为二值图像（每个像素点仅有1 和0 两个取值可能）"
                ,"tmp/debug_threshold.jpg");
        ...
        functionControler.recordProcess("使用闭操作。对图像进行闭操作以后，可以看到车牌区域被连接成一个矩形装的区域。"
                , "tmp/debug_morphology.jpg");

        // 求轮廓。求出图中所有的轮廓。这个算法会把全图的轮廓都计算出来，因此要进行筛选。
        ...
        functionControler.recordProcess("求轮廓。求出图中所有的轮廓。这个算法会把全图的轮廓都计算出来，因此要进行筛选。"
            ,"tmp/debug_Contours.jpg");
        ...
        MatVector rectsContours = new MatVector();
        ...
        for (int i = 0; i < rects.size(); i++) {
            ...
            if (verifySizes(minRect)) {
                ...
                // 旋转这部分代码确实可以将某些倾斜的车牌调整正，但是它也会误将更多正的车牌搞成倾斜！
        ...
        functionControler.recordProcess("对车牌进行倾斜矫正后最终结果","tmp/debug_result.jpg");
    ...
    // 是否开启测试模式
    protected boolean testMode = false;//DEFAULT_DEBUG;
}
